...
function riskBlock() {
  ...
  const imp = String(g(rs, 'impact')).toLowerCase();
  const lik = String(g(rs, 'likelihood')).toLowerCase();
  const impN = { limited: 2, serious: 3, severe: 5 }[imp];
  const likN = { rare: 2, possible: 3, likely: 5 }[lik];
  if (impN == null || likN == null) return '';
  const score = impN * likN;
  let band;
  if (score <= 6) band = ['Low', 'low'];
  else if (score <= 12) band = ['Moderate', 'moderate'];
  else if (score <= 18) band = ['High', 'high'];
  else band = ['Critical', 'critical'];
  const pos = Math.round(score / 25 * 100);
  const impLabel = { limited: 'Limited', serious: 'Serious', severe: 'Severe' }[imp];
  const likLabel = { rare: 'Rare', possible: 'Possible', likely: 'Likely' }[lik];
  let html = '<section class="block"><h2>Risk rating</h2><p class="lead">A qualitative read of this decision: impact against likelihood, mapped to an exposure score. It rates the decision or change, not only a vulnerability.</p>'
    + '<div class="expo"><div class="expohead">Risk exposure &middot; <b>' + score + '/25</b> &middot; <span class="expoband band-' + band[1] + '">' + band[0] + '</span></div>'
    + '<div class="expobar"><span class="expomark" style="left:' + pos + '%"></span></div>'
    + '<div class="expolabels"><span>Low</span><span>Moderate</span><span>High</span><span>Critical</span></div>'
    + '<p class="riskmeta2">Impact: ' + impLabel + ' &middot; Likelihood: ' + likLabel + '</p></div>';
  if (len(g(rs, 'rationale')) > 0) html += '<p class="riskwhy">' + e(rs.rationale) + '</p>';
