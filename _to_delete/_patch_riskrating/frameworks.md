...
**Score inherent and residual.** Rate each decision twice: inherent exposure (the current state, before the recommended response) and residual exposure (what remains after it is executed as intended). The report draws both as two markers on the same bar, so the gap is the visible value of the recommendation. When an adverse impact has already been observed or confirmed (files encrypted, an outage under way, data exposed), the risk has materialized: score its likelihood as Likely, never Possible. Rating an already-observed impact as merely Possible is the most common scoring error and makes an active incident read as far lower than it is.

**Scale granularity (known limitation).** With three levels per axis mapped to {2, 3, 5}, the only reachable scores are 4, 6, 9, 10, 15, and 25: there is no value between 16 and 24, and the High (13 to 18) and Critical (19 to 25) bands each contain a single reachable score (15 and 25). So "about 20 out of 25" cannot be expressed; at the top the honest choices are 15 or 25. If finer resolution is wanted, move both axes to a 1 to 5 scale (a standard 5x5 heat map): the report's exposure bar is already scored out of 25 and needs no change, only these labels and the number maps in the three report generators (report.js, report.sh, report.py) do.
