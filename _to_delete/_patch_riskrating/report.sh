...
+ ((.risk_score // {}) as $rs
   | def sc(o): (o) as $o
       ...
       | if ($in != null and $ln != null)
         then ($in*$ln) as $n
           | {n:$n,
              band:(if $n<=6 then ["Low","low"] elif $n<=12 then ["Moderate","moderate"] elif $n<=18 then ["High","high"] else ["Critical","critical"] end),
              pos:(($n/25*100)|round),
              impL:({"limited":"Limited","serious":"Serious","severe":"Severe"}[$imp]),
              likL:({"rare":"Rare","possible":"Possible","likely":"Likely"}[$lik]),
              rat:($o.rationale // "")}
         else null end;
