...
const CSS = [
  ...
  'table.opts th,table.opts td{border:1px solid var(--border);padding:8px 10px;text-align:left;vertical-align:top;overflow-wrap:break-word;}',
...
function scoreOne(o) {
  ...
  let band;
  if (n <= 4) band = ['Low', 'low'];
  else if (n <= 9) band = ['Moderate', 'moderate'];
  else if (n <= 15) band = ['High', 'high'];
  else band = ['Critical', 'critical'];
  return {
    n: n, band: band, pos: Math.round(n / 25 * 100),
    impLabel: { negligible: 'Negligible', minor: 'Minor', limited: 'Limited', moderate: 'Moderate', serious: 'Serious', major: 'Major', severe: 'Severe' }[imp],
    likLabel: { rare: 'Rare', unlikely: 'Unlikely', possible: 'Possible', likely: 'Likely', 'almost certain': 'Almost certain', 'almost-certain': 'Almost certain', certain: 'Almost certain' }[lik],
...
let out = '<!doctype html><html lang="en"><head><meta charset="utf-8">'
  ...
  + '<h1>' + e(question) + '</h1>'
  + (len(g(run, 'subtitle')) > 0 ? '<p class="subtitle">' + e(run.subtitle) + '</p>' : '')
